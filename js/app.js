...
/* ═══════════════════════════════════════════════════
   RENDER
═══════════════════════════════════════════════════ */
/* snapshot anterior para mostrar variación por ítem */
let snapAntes = [];
...
function render() {
  ...
  items.forEach((item, idx) => {
    ...
    /* ── Variación respecto al snapshot anterior ── */
    const antes      = snapAntes[idx];
    const dQty       = antes ? item.qty   - antes.qty   : 0;
    const dParcial   = antes ? p - (antes.qty * antes.price) : 0;
    const cambioPrc  = antes ? Math.abs(antes.price - item.price) > 0.0001 : false;
    const tieneVar   = antes && (Math.abs(dQty) > 0.0001 || Math.abs(dParcial) > 0.001 || cambioPrc);
    const sube       = dParcial >= 0;

    let varHTML = '';
    if (tieneVar) {
      const signoQty = dQty >= 0 ? '+' : '';
      const signoPar = dParcial >= 0 ? '+' : '';
      const clsVar   = sube ? 'var-up' : 'var-down';
      const icoVar   = sube ? 'ti-trending-up' : 'ti-trending-down';
      varHTML = `
        <div class="item-variacion ${clsVar}">
          <i class="ti ${icoVar}"></i>
          <span class="var-qty">Cant: ${signoQty}${rawNum(dQty, isInt?0:decimals)}</span>
          <span class="var-sep">·</span>
          <span class="var-par">Parcial: ${signoPar}${fmtDisp(dParcial)} Bs</span>
          ${cambioPrc ? `<span class="var-sep">·</span><span class="var-precio"><i class="ti ti-coin"></i> Precio: ${rawNum(antes.price,4)} → ${rawNum(item.price,4)}</span>` : ''}
        </div>`;
    }
    ...
    card.innerHTML = `
      <div class="item-header">
        ...
        <div class="item-badges">
          ...
          ${isInt       ? '<span class="badge-int" title="Unidad entera">N°</span>' : ''}
          ${item.price === 0 ? '<span class="badge-warn">⚠ precio 0</span>' : ''}
        </div>
        <div class="item-icons">
          <button class="icon-btn ${item.locked ? 'active' : ''}"
            onclick="toggleLock(${idx})" title="${item.locked ? 'Desbloquear' : 'Bloquear'}">
      ...
      <div class="item-body">
        ...
        <div class="item-parcial">
          <div>
            ...
            ${tieneVar ? `<span class="parc-antes">${fmtDisp(antes.qty * antes.price)} Bs</span>` : ''}
          </div>
...
═══════════════════════════════════════════════════ */
/* ═══════════════════════════════════════════════════
   MOTOR DE CIERRE EXACTO CON 2 DECIMALES
   Trabaja en centavos (enteros) para evitar errores
   de punto flotante. 4 estrategias en cascada.
═══════════════════════════════════════════════════ */

function cts(n)          { return Math.round(n * 100); }
function deCts(n)        { return n / 100; }
function parcialCts(it)  {
  const q = esUnidadEntera(it.unit) ? Math.round(it.qty) : rawNum(it.qty, decimals);
  return cts(rawNum(q * it.price, 2));
}
function totalCts()      { return items.reduce((s, i) => s + parcialCts(i), 0); }

function cerrarExacto(targetCts, cands) {

  /* A — Ajustar cantidad de ítems con decimales libres */
  const decLibres = cands
    .filter(i => !esUnidadEntera(items[i].unit) && items[i].price > 0)
    .sort((a, b) => parcialCts(items[b]) - parcialCts(items[a]));

  for (const i of decLibres) {
    const res = targetCts - totalCts();
    if (res === 0) break;
    const it = items[i];
    const parcialObj = (parcialCts(it) + res) / 100;
    if (parcialObj <= 0) continue;
    const qNueva = rawNum(parcialObj / it.price, decimals);
    const mn = parseFloat(it.minQty), mx = parseFloat(it.maxQty);
    let qf = qNueva;
    if (!isNaN(mn) && qf < mn) qf = mn;
    if (!isNaN(mx) && qf > mx) qf = mx;
    it.qty = qf;
  }

  /* B — Redistribuir ±0.01 Bs qty entre ítems decimales */
  let resB = targetCts - totalCts();
  if (resB !== 0 && decLibres.length > 0) {
    const dir   = resB > 0 ? 1 : -1;
    let pasos   = Math.abs(resB);
    let vuelta  = 0;
    while (pasos > 0 && vuelta < pasos * decLibres.length + 10) {
      vuelta++;
      for (const i of decLibres) {
        if (pasos <= 0) break;
        const it    = items[i];
        const step  = rawNum(0.01 * dir / it.price, decimals + 2);
        const qN    = rawNum(it.qty + step, decimals);
        const mn = parseFloat(it.minQty), mx = parseFloat(it.maxQty);
        if (!isNaN(mn) && qN < mn) continue;
        if (!isNaN(mx) && qN > mx) continue;
        if (qN <= 0) continue;
        const antes = parcialCts(it);
        it.qty = qN;
        pasos -= Math.abs(parcialCts(it) - antes);
      }
    }
  }

  /* C — Ajustar precio unitario (4 dec) del ítem de mayor peso */
  let resC = targetCts - totalCts();
  if (resC !== 0) {
    const porPeso = cands
      .filter(i => items[i].qty > 0 && items[i].price > 0)
      .sort((a, b) => parcialCts(items[b]) - parcialCts(items[a]));
    for (const i of porPeso) {
      if (resC === 0) break;
      const it = items[i];
      const pObj = (parcialCts(it) + resC) / 100;
      const pN   = rawNum(pObj / it.qty, 4);
      if (pN <= 0) continue;
      const ganado = cts(rawNum(it.qty * pN, 2)) - parcialCts(it);
      it.price = pN;
      it._precioAjustado = true;
      resC -= ganado;
    }
  }

  /* D — Último recurso: forzar parcial exacto en mayor peso */
  const resD = targetCts - totalCts();
  if (resD !== 0) {
    const porPeso = cands
      .filter(i => items[i].qty > 0 && items[i].price > 0)
      .sort((a, b) => parcialCts(items[b]) - parcialCts(items[a]));
    if (porPeso.length) {
      const it = items[porPeso[0]];
      it.price = rawNum((parcialCts(it) + resD) / 100 / it.qty, 4);
      it._precioAjustado = true;
    }
  }
}
...
function calcAjuste() {
  ...
  /* Guardar snapshot ANTES del ajuste para mostrar variaciones */
  snapAntes = items.map(it => ({ qty: it.qty, price: it.price }));

  const snapBefore  = items.map(it => ({ ...it }));
  const totalBefore = total;

  /* Candidatos: no bloqueados, precio > 0, participan en el método */
  const candidatos = items
    .map((it, i) => i)
    .filter(i => !items[i].locked && items[i].price > 0 &&
                 (method === 'prop' || method === 'pct' || checked[i] !== false));

  /* ── PASO 1: ajuste proporcional inicial ── */
  if (method === 'prop') {
    const f = total > 0 ? target / total : 1;
    items.forEach(item => { if (!item.locked) item.qty = applyConditions(item, item.qty * f); });
    ...
    } else { showToast('Sin margen con los seleccionados.', 'err'); return; }

  } else if (method === 'big') {
    const idxs = [...candidatos].sort((a, b) => items[b].qty * items[b].price - items[a].qty * items[a].price);
    let rem = target - total;
    for (const i of idxs) {
      if (Math.abs(rem) < 0.005) break;
      const it = items[i];
    ...
    const f   = 1 + pct / 100;
    items.forEach((item, i) => { if (!item.locked && checked[i] !== false) item.qty = applyConditions(item, item.qty * f); });
  }

  /* ── PASO 2: cerrar exactamente al objetivo ── */
  if (method !== 'pct') {
    cerrarExacto(targetCents, candidatos);
  }
  ...
  /* ── Mensaje ── */
  const diffCts          = Math.abs(totalCts() - targetCents);
  const preciosAjustados = items.filter(i => i._precioAjustado);
  items.forEach(i => delete i._precioAjustado);

  if (method === 'pct') {
    showToast('Ajuste por porcentaje. Total: ' + fmtDisp(getTotal()) + ' Bs');
  } else if (diffCts === 0) {
    if (preciosAjustados.length > 0) {
      showToast('¡Objetivo logrado! Se ajustó el precio unitario de "' + preciosAjustados[0].desc.substring(0,28) + '..." para cuadrar exactamente.');
    } else {
      showToast('¡Diferencia cero! Monto objetivo logrado exactamente.');
    }
  } else {
    showToast('Diferencia mínima: ' + fmtDisp(deCts(diffCts)) + ' Bs — revisa bloqueos o condiciones.', 'warn');
  }
}
